...
def classify_trend(dby: float, y: float, today: float) -> str:
    ...
    if prev_good and today_bad:
        return "GOOD_THEN_BAD"
    if prev_bad and today_good:
...
def apply_trend(mult: float, trend: str) -> float:
    ...
    match trend:
        case "BETTER":
            return strong if is_up else mild
        case "MAINTAINING":
            return mild
        case "WORSE":
            return mild
        case "WORSEN":
            return strong if is_down else mild
        case "GOOD_THEN_BAD":
            return mild if is_down else 0.9
        case "BAD_THEN_GOOD":
            return mild if is_up else 1.1
        case "INSUFFICIENT":
            return mult
        case _:
            return mult
...
def apply_conversion_to_budget(
    budget: float, y_results: float, has_conversion: bool, conversion_raw: float
) -> float:
...
def conversion_tag(raw_budget: float, final_budget: float) -> str:
    if round(raw_budget) == round(final_budget):
        return ""
...
def worse_200_250(budget: int, t_spend: float) -> dict:
    if budget <= 4000:
        return {
            "action": "SET_BUDGET",
            "new_budget": max(t_spend, FLOOR),
            "reason": "budget=amount_spent",
        }
    if budget <= 10000:
        return {"action": "SET_BUDGET", "new_budget": budget * 0.5, "reason": "cut_50"}
    if budget <= 15000:
        return {"action": "SET_BUDGET", "new_budget": budget * 0.6, "reason": "cut_40"}
    if budget <= 20000:
        return {"action": "SET_BUDGET", "new_budget": budget * 0.65, "reason": "cut_35"}
    return {"action": "SET_BUDGET", "new_budget": budget * 0.7, "reason": "cut_30"}


def worsen_250_plus(budget: int, t_spend: float) -> dict:
    if budget <= 4000:
        return {"action": "PAUSE", "new_budget": None, "reason": "pause"}
    if budget <= 10000:
        return {
            "action": "SET_BUDGET",
            "new_budget": max(t_spend, FLOOR),
    ...
    if budget <= 15000:
        return {"action": "SET_BUDGET", "new_budget": budget * 0.4, "reason": "cut_60"}
    if budget <= 20000:
        return {"action": "SET_BUDGET", "new_budget": budget * 0.5, "reason": "cut_50"}
    return {"action": "SET_BUDGET", "new_budget": budget * 0.6, "reason": "cut_40"}
...
def _result(action: str, new_budget: int | None, reason: str, steps: list | None) -> DecisionResult:
    return DecisionResult(action=action, new_budget=new_budget, reason=reason, steps=steps or [])


def _build_budget_decision(
    ctx: DecisionContext, base_mult: float, reason_base: str, steps: list | None
) -> DecisionResult:
    ...
    after_conv = apply_conversion_to_budget(rec, ctx.y_results, ctx.has_conversion, ctx.conversion)
...
def _pause_or_fallback(
    ctx: DecisionContext, reason_base: str, steps: list | None
) -> DecisionResult:
    ...
    if db == "D1":
        ...
        if s == "AAA":
            ...
            if band == "bad":
                ...
                return _build_budget_decision(
                    ctx, 1 - cut, f"AAA_D1_BAD_CUT_{round(cut * 100)}", steps
                )
            if band == "worse":
                cut = interp(t, 200, 250, 0.20, 0.40)
                return _build_budget_decision(
                    ctx, 1 - cut, f"AAA_D1_WORSE_CUT_{round(cut * 100)}", steps
                )
            if band == "worsen":
    ...
    if db in ("D2", "D3+"):
        ...
        if effective_band == "bad":
            if t <= 175:
                ...
                return _build_budget_decision(
                    ctx, 1 - cut, f"{s}_{db}_BAD_150_175_CUT_{round(cut * 100)}", steps
                )
            else:
                ...
                return _build_budget_decision(
                    ctx, 1 - cut, f"{s}_{db}_BAD_176_200_CUT_{round(cut * 100)}", steps
                )
        ...
        return _build_budget_decision(
            ctx, trend_mult, f"{s}_{db}_{effective_band.upper()}_TREND_{trend}", steps
        )
